...
static void mdss_mdp_kcal_update_pcc(struct mdss_mdp_ctl *ctl,
		struct kcal_lut_data *lut_data)
{
	...
	mdss_mdp_pcc_config(ctl, &pcc_config, &copyback);
}

static void mdss_mdp_kcal_read_pcc(struct mdss_mdp_ctl *ctl,
		struct kcal_lut_data *lut_data)
{
	u32 copyback = 0;
	struct mdp_pcc_cfg_data pcc_config;
...
static void mdss_mdp_kcal_update_pa(struct mdss_mdp_ctl *ctl,
		struct kcal_lut_data *lut_data)
{
	u32 copyback = 0;
	...
	mdss_mdp_pa_config(ctl, &pa_config, &copyback);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (mdss_mdp_kcal_get_ctl(&ctl)) {
		mdss_mdp_kcal_update_pcc(ctl, lut_data);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (mdss_mdp_kcal_get_ctl(&ctl))
		mdss_mdp_kcal_read_pcc(ctl, lut_data);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (mdss_mdp_kcal_get_ctl(&ctl)) {
		mdss_mdp_kcal_update_pcc(ctl, lut_data);
		mdss_mdp_kcal_update_pa(ctl, lut_data);
		mdss_mdp_kcal_display_commit(ctl);
	}
